...
class NoteControllerTest extends WebLayerTestSupport {
    ...
    @Test
    @DisplayName("POST /api/notes returns 201 with the created note")
    void createNoteReturnsCreated() throws Exception {
        ...
        when(noteService.createNoteFromRequest(any(NoteRequestBody.class))).thenReturn(response());
    ...
    @Test
    @DisplayName("POST /api/notes returns 400 listing the field when the title is blank")
    void createNoteRejectsBlankTitle() throws Exception {
        NoteRequestBody request = new NoteRequestBody("   ", "body text", USER_ID, Set.of());

        mockMvc.perform(post("/api/notes")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.fields").value("title"))
                .andExpect(jsonPath("$.fieldsMessage").value("Title is required"));
    }

    @Test
    @DisplayName("POST /api/notes returns 400 listing the field when userId is absent")
    void createNoteRejectsMissingUserId() throws Exception {
        NoteRequestBody request = new NoteRequestBody("first", "body text", null, Set.of());
        ...
        mockMvc.perform(post("/api/notes")
                ...
                .andExpect(jsonPath("$.fieldsMessage").value("User ID is required"));
    }
    ...
    @Test
    @DisplayName("POST /api/notes returns 400 when the service rejects an unknown tag")
    void createNoteTranslatesUnknownTag() throws Exception {
        NoteRequestBody request = new NoteRequestBody("first", "body text", USER_ID, Set.of(TAG_ID));
        when(noteService.createNoteFromRequest(any(NoteRequestBody.class)))
    ...
    @Test
    @DisplayName("PUT /api/notes/{id} returns 200 with the updated note")
    void updateNoteReturnsOk() throws Exception {
        NoteRequestBody request = new NoteRequestBody("first", "body text", USER_ID, Set.of());
        when(noteService.updateNoteFromRequest(eq(NOTE_ID), any(NoteRequestBody.class))).thenReturn(response());
    ...
    @Test
    @DisplayName("PUT /api/notes/{id} returns 400 when the title is blank")
    void updateNoteRejectsBlankTitle() throws Exception {
        NoteRequestBody request = new NoteRequestBody("", "body text", USER_ID, Set.of());
