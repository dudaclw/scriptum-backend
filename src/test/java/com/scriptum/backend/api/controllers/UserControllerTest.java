...
class UserControllerTest extends WebLayerTestSupport {
    ...
    void updateUserReturnsOk() throws Exception {
        UserRequestBody request = new UserRequestBody("New Name", EMAIL, "secret123", null);
        when(userService.updateUser(eq(USER_ID), any(UserRequestBody.class))).thenReturn(response());
    ...
    void updateUserRejectsMalformedEmail() throws Exception {
        UserRequestBody request = new UserRequestBody("New Name", "not-an-email", "secret123", null);
    ...
    void updateUserRejectsShortPassword() throws Exception {
        UserRequestBody request = new UserRequestBody("New Name", EMAIL, "abc", null);
        ...
        mockMvc.perform(put("/api/users/{id}", USER_ID)
                ...
                .andExpect(jsonPath("$.fieldsMessage").value("Password must be at least 6 characters"));
    }

    @Test
    @DisplayName("PUT /api/users/{id} returns 400 when the name is blank")
    void updateUserRejectsBlankName() throws Exception {
        UserRequestBody request = new UserRequestBody("   ", EMAIL, "secret123", null);
